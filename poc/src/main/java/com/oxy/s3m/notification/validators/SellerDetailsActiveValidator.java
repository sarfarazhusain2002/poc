...
public class SellerDetailsActiveValidator {
	...
	public JSONObject validate(JSONObject custobj){
	    ...
	    if(custobj.getString("email")==null || custobj.getString("email").equals("")){
	    	...
	    	System.out.println("email>>>"+this.validateEmail(custobj.getString("email").toString()));
	    	if(!this.validateEmail(custobj.getString("email").toString())){
	    	obj.put("email", "invalid email address");
	    	obj.put("validate", false);	    	
	    	}
	    }
	    if(custobj.getString("active")==null || custobj.getString("active").equals("")){
	    	obj.put("active", "fcmToken Can not be blank");
	    	obj.put("validate", false);
	    }else if(custobj.getString("active").length()>1){
	    	obj.put("active", "It can be either 0 or 1");
	    	obj.put("validate", false);	    	
	    }
	    
	    if(custobj.getString("sellerId")==null || custobj.getString("sellerId").equals("")){
	    	obj.put("sellerId", "sellerId Can not be blank");
	    	obj.put("validate", false);
	    }
		return obj;
	}
